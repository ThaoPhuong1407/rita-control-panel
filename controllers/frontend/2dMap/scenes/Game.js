...
// Player movement
function playerMovement(data, playerN, color, playerImage, infoCoord, scene) {
    const movementsArr = data.player_pos;
    ...
    for (let i = 0; i < movementsArr.length - 1; i+=1) {
        ...
        if (i === movementsArr.length - 2) {
            ...
            // if (color === '0x6aab9c') {
            //     console.log(`Green is at ${movementsArr[i+1][0]}, ${movementsArr[i+1][1]}`)
            // }
...
function playerMovementColor(color, data, playerN, infoCoord, scene) {
    if (color) {
        color = color.toLowerCase();
        if (color === 'red') {
           playerMovement(data, playerN, red, 'redPlayer', infoCoord, scene);
        } else if (color === 'blue') {
           playerMovement(data, playerN, blue, 'bluePlayer', infoCoord, scene);
        } else if (color === 'green') {
           playerMovement(data, playerN, green, 'greenPlayer', infoCoord, scene);
        }
    } else {
        // For old data
        const selectedColor = colorArr.pop();
        const selectedPlayer = playerArr.pop();
       playerMovement(data, playerN, selectedColor, selectedPlayer, infoCoord, scene);
    }
}
...
function addBlockage(data, scene, block_type, imageName) {
    data.forEach(blockage => {
        ...
        if (blockage.block_type === block_type) {
            console.log(`Info: added ${blockage.block_type}`);
...
function initializedBaseMap(map_type, scene, original_objs) {  
    ...
    if (map_type.toLowerCase().includes('sparky')) {
        ...
        basemap = 'Sparky';
    } else if (map_type.toLowerCase().includes('saturn1')) { 
        scene.map_coordinates = Map.map_coordinates.Saturn1;
        basemap = 'Saturn1';
    } else if (map_type.toLowerCase().includes('saturn')) {
        scene.map_coordinates = Map.map_coordinates.Saturn;
        basemap = 'Saturn';
    ...
    console.log('Base map selected:', basemap);
...
class Game extends Phaser.Scene {
	constructor() {
        ...
        this.connector = {                      // Stores the connecting point(Smoother UI - lines)
            'player1' : {'x': 0, 'z': 0},
            'player2' : {'x': 0, 'z': 0},
            'player3' : {'x': 0, 'z': 0},
            'robot'   : {'x': 0, 'z': 0},
        ...
        this.graphics = {                        // Stores the graphic object: line (for player1's movement)
            'player1' : null,
            'player2' : null,
            'player3' : null,
            'robot'   : null,
    ...
    destroyAndDeleteSprite(obj) {
        for (const key in obj) {
            this.obj[key].destroy();  // remove the sprite from the scene
    ...
    //             this.obj[key].destroy();  // remove the sprite from the scen
    //             this.obj[key] = null;     // Reset value to null
    //         } else {
    //             return;
    //         }
    //     }
    ...
    resetGame() {
        ...
        this.connector_player1 = {'x': 0, 'z': 0};      // Stores the connecting point(Smoother UI - lines)
        this.connector_player2 = {'x': 0, 'z': 0};      // Stores the connecting point(Smoother UI - lines)
        this.connector_player3 = {'x': 0, 'z': 0};      // Stores the connecting point(Smoother UI - lines)
        this.connector_robot = {'x': 0, 'z': 0};        // Stores the connecting point

        if (this.graphics_robot != null) {
            this.graphics_robot.destroy();
            this.graphics_robot = null;
        }

        if (this.graphics_player1 != null) {
            this.graphics_player1.destroy();
            this.graphics_player1 = null;
        }

        if (this.graphics_player2 != null) {
            this.graphics_player2.destroy();
            this.graphics_player2 = null;
        }
        if (this.graphics_player3 != null) {
            this.graphics_player3.destroy();
            this.graphics_player3 = null;
        }
        ...
        if (this.spritePool.image_player.player1 != null) {
            this.spritePool.image_player.player1.destroy();
            this.spritePool.image_player.player1 = null;
        }

        if (this.spritePool.image_player.player2 != null) {
            this.spritePool.image_player.player2.destroy();
            this.pritePool.image_player.player2 = null;
        }

        if (this.spritePool.image_player.player3 != null) {
            this.spritePool.image_player.player3.destroy();
            this.pritePool.image_player.player3 = null;
        }
        if (this.spritePool.image_player.player1Role != null) {
            this.spritePool.image_player.player1.destroy();
            this.spritePool.image_player.player1 = null;
        }

        if (this.spritePool.image_player.player2Role != null) {
            this.spritePool.image_player.player2.destroy();
            this.pritePool.image_player.player2 = null;
        }

        if (this.spritePool.image_player.player3Role != null) {
            this.spritePool.image_player.player3.destroy();
            this.pritePool.image_player.player3 = null;
        }

        if (this.spritePool.text_role.player1 != null) {
            this.spritePool.text_role.player1.destroy();
            this.spritePool.text_role.player1 = null;
        }
        if (this.spritePool.text_role.player2 != null) {
            this.spritePool.text_role.player2.destroy();
            this.spritePool.text_role.player2 = null;
        }
        if (this.spritePool.text_role.player3 != null) {
            this.spritePool.text_role.player3.destroy();
            this.spritePool.text_role.player3 = null;
        }
    ...
    create() {
        ...
        let scale_x = 0;        // Scale the x-coordinate of an object 
        let scale_z = 0;        // Scale the z-coordinate of an object

        //------- The initial display before adding the actual map
        const style = { font: "bold 50px Arial", fill: "#303030"};
        const styleRole = { font: "23px Arial", fill: "#f8f7fc"}; 
        const input_text = scene.add.text(200, 450, "Waiting for incoming data", style).setVisible(true);
        ...
        Map.socketFrontEnd.on(Map.socketKey.trial_start, function (data) {   
            ...
            initializedBaseMap(data.map_name, scene, input_text);  
            ...
            // otherwise, we don't have callsign yet, assign color randomly in the playerMovementColor function
        });
        ...
        Map.socketFrontEnd.on(Map.socketKey.groundtruth_victim_list, function (data) {  
            data.forEach(victim => {
                console.log(`Info: added victims`);
                scale_x = scaleXCoordinate(victim.x, scene);
                scale_z = scaleZCoordinate(victim.z, scene); 

                if (victim.block_type === "block_victim_1") { // green
                    Map.victims.regular += 1;
                    const regular_victim_sprite = scene.add.sprite(scale_x, scale_z, 'green_victim').setOrigin(0, 0);
                    if (data.unique_id) { // For data version 0.6 ("testbed_version": "2.0.0-dev.361-872fa66")
                        scene.spritePool.image_green_victim[data.unique_id] = regular_victim_sprite;
                    } else {
                        scene.spritePool.image_green_victim[`${scale_x},${scale_z}`] = regular_victim_sprite;
                    }
                    
                } else if (victim.block_type === "block_victim_2" || victim.block_type === 'block_victim_proximity') { // yellow
                    Map.victims.critical += 1;
                    const critical_victim_sprite = scene.add.sprite(scale_x, scale_z, 'yellow_victim').setOrigin(0, 0);
                    if (data.unique_id) { // For data version 0.6 ("testbed_version": "2.0.0-dev.361-872fa66")
                        scene.spritePool.image_yellow_victim[data.unique_id] = critical_victim_sprite;
                    } else {
                        scene.spritePool.image_yellow_victim[`${scale_x},${scale_z}`] = critical_victim_sprite;
                    }
                }
            });
        });
        ...
        Map.socketFrontEnd.on(Map.socketKey.groundtruth_victims_expired, function (data) {  
            ...
            console.log(`Info: yellow victims timeout ${data}`);
        ...
        Map.socketFrontEnd.on(Map.socketKey.event_door, function (state_and_coordinates) {  
            const {z} = state_and_coordinates.coordinates;
            const {x} = state_and_coordinates.coordinates;
            scale_x = scaleXCoordinate(x, scene);
            scale_z = scaleZCoordinate(z, scene); 
            const key_string = `${scale_x},${scale_z}`;

            if (state_and_coordinates.state) {
                console.log(`Event Door: ${state_and_coordinates.playername} opens door`);
                ...
                console.log(`Event Door: ${state_and_coordinates.playername} closes door`);
        ...
        Map.socketFrontEnd.on(Map.socketKey.event_triage, function (state_and_coordinates) {  
            if (state_and_coordinates.state === 'SUCCESSFUL') {
                const {z} = state_and_coordinates.victim_location;
                const {x} = state_and_coordinates.victim_location;
                scale_x = scaleXCoordinate(x, scene);
                scale_z = scaleZCoordinate(z, scene); 
        
                if (state_and_coordinates.victim_type.toLowerCase() === 'regular') {
                    console.log(`Event Triage: ${state_and_coordinates.playername} successfully triaged a ${state_and_coordinates.victim_type} victim`);
                     // add the sprite from the scene
                    const triaged_green = scene.add.sprite(scale_x, scale_z, 'green_victim_triaged').setOrigin(0, 0);
                    scene.spritePool.image_triaged_green_victim[`${scale_x},${scale_z}`] = triaged_green;

                    // remove the sprite from the scene
                    scene.spritePool.image_green_victim[`${scale_x},${scale_z}`].destroy();  
                    delete scene.spritePool.image_green_victim[`${scale_x},${scale_z}`];
                    
                } else {
                    console.log(`Event Triage: ${state_and_coordinates.playername} successfully triaged a ${state_and_coordinates.victim_type} victim`);
                    const triaged_yellow = scene.add.sprite(scale_x, scale_z, 'yellow_victim_triaged').setOrigin(0, 0);
                    scene.spritePool.image_triaged_yellow_victim[`${scale_x},${scale_z}`] = triaged_yellow;

                    // remove the sprite from the scene
                    scene.spritePool.image_yellow_victim[`${scale_x},${scale_z}`].destroy();  
                    delete scene.spritePool.image_yellow_victim[`${scale_x},${scale_z}`];
                }
            } else if (state_and_coordinates.state === 'UNSUCCESSFUL') {
                console.log(`Event Triage: ${state_and_coordinates.playername} failed to triage a ${state_and_coordinates.victim_type} victim`);
            } else if (state_and_coordinates.state === 'IN_PROGRESS') {
                console.log(`Event Triage: ${state_and_coordinates.playername} is triaging a ${state_and_coordinates.victim_type} victim`);
            }
        });

        // 3. Victim placed (EVENT)
        Map.socketFrontEnd.on(Map.socketKey.victimPlaced, function (name_location) {  
            const { playername } = name_location;
            const { victim_type } = name_location;
            const {z} = name_location.victim_location;
            const {x} = name_location.victim_location;
            scale_x = scaleXCoordinate(x, scene);
            scale_z = scaleZCoordinate(z, scene); 
            console.log(`Event: ${playername} placed a ${victim_type} victim`);

            if (victim_type.toLowerCase() === 'regular') {
                const regular_victim_sprite = scene.add.sprite(scale_x, scale_z, 'green_victim').setOrigin(0, 0);
                scene.spritePool.image_green_victim[`${scale_x},${scale_z}`] = regular_victim_sprite;
            } else {
                const critical_victim_sprite = scene.add.sprite(scale_x, scale_z, 'yellow_victim').setOrigin(0, 0);
        ...
        Map.socketFrontEnd.on(Map.socketKey.victimPickedUp, function (name_location) {  
            const { playername } = name_location;
            const { victim_type } = name_location;
            const {z} = name_location.victim_location;
            const {x} = name_location.victim_location;
            scale_x = scaleXCoordinate(x, scene);
            scale_z = scaleZCoordinate(z, scene); 
            console.log(`Event: ${playername} is carrying a ${victim_type} victim`);
        ...
        // 4. Rubble Destroyed 
        Map.socketFrontEnd.on(Map.socketKey.rubbleDestroyed, function (name_location) {  
            const { playername } = name_location;
            const {z} = name_location.rubbleLocation;
            const {x} = name_location.rubbleLocation; 
            const {y} = name_location.rubbleLocation; 
            scale_x = scaleXCoordinate(x, scene);
            scale_z = scaleZCoordinate(z, scene); 
    
            if (y === selected_floor && scene.spritePool.image_blockage[`${scale_x},${scale_z}`]) {
                console.log(`Event Rubble: ${playername} destroyed a rubble`);
        ...
        // 5. Frozen  
        Map.socketFrontEnd.on(Map.socketKey.frozen, function (name_location) {  
            const { playername, frozen, medic_playername } = name_location;
            const { x, z } = name_location.location;
            scale_x = scaleXCoordinate(x, scene);
            scale_z = scaleZCoordinate(z, scene); 
            const key = `${scale_x},${scale_z}`;

            if (frozen) {
                console.log(`Event Frozen: ${playername} is frozen at (x: ${x}, z: ${z}).`);
                ...
                console.log(`Event Frozen: ${playername} is unfrozen by ${medic_playername}, at (x: ${x}, z: ${z})`);
                scene.spritePool.image_frozen[key].destroy();  
                delete scene.spritePool.image_frozen[key];   
            } 
        });

        // 6. Player changes role 
        Map.socketFrontEnd.on(Map.socketKey.event_roleChange, function (data) { 
            const { playerNum, playername, new_role, prev_role } = data;
            console.log(`${playername} changes role from ${prev_role} to ${new_role}`) 
        ...
        // 7. Player's movements
        Map.socketFrontEnd.on(Map.socketKey.observation_player1, function (data) {  
             const color = data.playerData.callsign;
             playerMovementColor(color, data, 'player1', 500, scene);
        });

        Map.socketFrontEnd.on(Map.socketKey.observation_player2, function (data) {  
            const color = data.playerData.callsign;
            playerMovementColor(color, data, 'player2', 1000, scene);
        });

        Map.socketFrontEnd.on(Map.socketKey.observation_player3, function (data) {  
            const color = data.playerData.callsign;
            playerMovementColor(color, data, 'player3', 1500, scene);
        });

        // ROBOT MOVEMENT
        Map.socketFrontEnd.on(Map.socketKey.robot_pos, function(data) {
        ...
        Map.socketFrontEnd.on(Map.socketKey.next_room, function(prediction_and_time) {
